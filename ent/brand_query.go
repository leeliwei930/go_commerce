...
func (bq *BrandQuery) QueryProducts() *ProductQuery {
...
func (bq *BrandQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (bq *BrandQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (bq *BrandQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (bq *BrandQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (bq *BrandQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
...
func (bq *BrandQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := bq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (bq *BrandQuery) WithProducts(opts ...func(*ProductQuery)) *BrandQuery {
	query := &ProductQuery{config: bq.config}
...
func (bq *BrandQuery) sqlAll(ctx context.Context) ([]*Brand, error) {
	...
	if query := bq.withProducts; query != nil {
		...
		nodeids := make(map[uuid.UUID]*Brand)
		for i := range nodes {
		...
		query.Where(predicate.Product(func(s *sql.Selector) {
			s.Where(sql.InValues(brand.ProductsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.brand_products
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "brand_products" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "brand_products" returned %v for node %v`, *fk, n.ID)
			}
